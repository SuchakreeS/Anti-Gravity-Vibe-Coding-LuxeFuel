  ...
  return (
    <div className="lg:col-span-2 flex flex-col gap-6">
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.1 }} className="card bg-carbon border border-industrial-border shadow-2xl">
        <div className="card-body">
          <h2 className="card-title text-text-secondary text-xs uppercase tracking-widest mb-4">2. Gas Price Refueled / Time ({currency})</h2>
          <div className="h-64 overflow-visible">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }} className="card bg-carbon border border-industrial-border shadow-2xl">
        <div className="card-body">
          <h2 className="card-title text-text-secondary text-xs uppercase tracking-widest mb-4">3. Gas Price Per Litre ({currency})</h2>
          <div className="h-64 overflow-visible">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={convertedRecords} margin={{ top: 20, right: 50, left: 10, bottom: 0 }}>
                <defs>
                ...
                <Tooltip
                  ...
                  allowEscapeViewBox={{ x: false, y: true }}
                />
                <Area isAnimationActive={false} type="monotone" dataKey="convertedPricePerLitre" name={`${currency} / L`} stroke="#F97316" fillOpacity={1} fill="url(#colorPrice)" strokeWidth={4} dot={{ r: 3, fill: "#F97316", strokeWidth: 0, opacity: 1 }} activeDot={{ r: 6, strokeWidth: 0, fill: "#F97316" }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
